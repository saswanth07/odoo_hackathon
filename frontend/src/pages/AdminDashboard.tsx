  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10 py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: "Total Users", value: analytics?.totalUsers || "0", growth: "+12.5%", icon: Users, color: "text-primary", bg: "bg-primary/10" },
          { label: "Active Trips", value: analytics?.totalTrips || "0", growth: "+8.2%", icon: Globe, color: "text-accent", bg: "bg-accent/10" },
          { label: "Platform Revenue", value: `$${(analytics?.totalEstimatedCost || 0).toLocaleString()}`, growth: "+15.3%", icon: TrendingUp, color: "text-success", bg: "bg-success/10" },
          { label: "Top Regions", value: Object.keys(analytics?.topCities || {}).length || "0", growth: "-2.1%", icon: Zap, color: "text-warning", bg: "bg-warning/10" },
        ].map((stat, i) => (
          <motion.div 
            key={i}
      ...
      <Tabs defaultValue="users" className="space-y-8">
            ...
            <Card className="bg-card/40 backdrop-blur-md border-border/40 rounded-3xl p-8 shadow-2xl">
              ...
              <div className="h-[300px] w-full flex items-center justify-center">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      data={analytics ? Object.entries(analytics.topCities).map(([name, value]) => ({ name, value })) : pieData}
                      cx="50%"
                      cy="50%"
                      innerRadius={80}
                      outerRadius={120}
        ...
        <TabsContent value="sites" className="outline-none">
           <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { name: "Kyoto Temples", visitors: "12.4k", rating: 4.9, img: "https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?q=80&w=400" },
                { name: "Eiffel Tower", visitors: "18.2k", rating: 4.8, img: "https://images.unsplash.com/photo-1502602898657-3e91760cbb34?q=80&w=400" },
                { name: "Bali Beaches", visitors: "9.8k", rating: 4.7, img: "https://images.unsplash.com/photo-1537996194471-e657df975ab4?q=80&w=400" },
              ].map((site, i) => (
                <Card key={i} className="overflow-hidden bg-card/40 border-border/40 rounded-3xl shadow-xl group cursor-pointer">
                  <div className="h-48 overflow-hidden">
                    <img src={site.img} alt={site.name} className="w-full h-full object-cover transition-transform group-hover:scale-110" />
                  </div>
