...
import { Search, Filter, SortDesc, Download, FileText, CheckCircle, Receipt, Calendar, User, Hash } from 'lucide-react';
...
// Expenses will be fetched from the backend budget analysis
...
export default function ExpenseInvoice() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Map expensive activities to the expense list
        const items = budgetData.expensiveActivities.map((act: any, idx: number) => ({
          id: idx + 1,
          category: act.type || "Activity",
          description: act.name,
          details: "Confirmed",
          unitCost: act.estimatedCost,
  ...
  if (loading) return <div className="flex items-center justify-center h-screen">Loading Invoice...</div>;
    ...
    <div className="max-w-5xl mx-auto space-y-10 py-6">
      ...
      <Card className="bg-card/40 backdrop-blur-md border-border/40 rounded-3xl overflow-hidden shadow-2xl relative">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-8">
            <div className="space-y-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-2xl font-black tracking-tight">{trip?.title}</h2>
                  <p className="text-xs text-muted-foreground font-medium uppercase tracking-widest">Invoice #TRP-2026-{tripId}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-x-12 gap-y-4 pt-2">
                ...
                <div className="space-y-1">
                  ...
                  <Badge className={`font-bold uppercase text-[9px] ${budget?.status === 'UNDER_BUDGET' ? 'bg-success/20 text-success border-success/30' : 'bg-warning/20 text-warning border-warning/30'}`}>
            ...
            <div className="bg-background/50 rounded-2xl p-6 min-w-[240px] flex flex-col justify-center border border-border/20">
              ...
              <p className="text-4xl font-black text-primary mb-4">${budget?.budgetLimit}</p>
              <div className="space-y-2">
                <div className="flex justify-between text-xs font-bold">
                  <span className="text-muted-foreground">Used</span>
                  <span>${budget?.totalEstimatedCost}</span>
                </div>
                <div className="h-1.5 w-full bg-secondary/50 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-primary rounded-full" 
                    style={{ width: `${Math.min((budget?.totalEstimatedCost / budget?.budgetLimit) * 100, 100)}%` }} 
                  />
                </div>
              </div>
            </div>
        ...
        <div className="p-8 bg-muted/20 flex flex-wrap gap-4 justify-between items-center border-t border-border/20">
          ...
          <Button 
            onClick={() => alert("Invoice marked as paid successfully!")}
            className="h-12 px-8 rounded-xl bg-primary hover:bg-primary/90 text-white font-black shadow-xl shadow-primary/20"
          >
